...
import { LABEL_SM, BTN_COMPACT, WorkflowNode } from "./types";
import { LayoutZone } from "../LayoutDesigner";


export default function NodePropertiesPanel({
  node, layouts, onUpdateNode, onDeleteNode,
}: {
  node: WorkflowNode;
  ...
  const updateConfig = (k: string, v: any) => onUpdateNode({ config: { ...node.config, [k]: v } });

  const handleFileSelect = (file: File | null, key = "__file") => {
    if (!file) { onUpdateNode({ config: { ...(node.config || {}), [key]: null, __fileDataUrl: null, __rows: null, filePath: "" } }); return; }
    const r = new FileReader();
    r.onload = () => onUpdateNode({ config: { ...(node.config || {}), [key]: file, __fileDataUrl: r.result, filePath: file.name } });
        ...
        return (
          <div className="space-y-2">
            <div>
              ...
              <input type="file" accept=".xls,.xlsx" onChange={(e) => handleFileSelect(e.target.files?.[0] || null)} className="w-full text-[12px]" />
            ...
            <div>
              ...
              <Input className="h-8 text-[12px]" value={node.config.sheetName || "Payslip,Earnings,Deductions"} onChange={(e) => updateConfig("sheetName", e.target.value)} placeholder="Payslip,Earnings,Deductions" />
            </div>
            <div className="flex items-center gap-2">
              <input type="checkbox" checked={node.config.hasHeaders || false} onChange={(e) => updateConfig("hasHeaders", e.target.checked)} />
        ...
        return (
          <div className="space-y-2">
            <div>
              ...
              <Select value={node.config.templateId || ""} onValueChange={(v) => updateConfig("templateId", v)}>
                ...
                <SelectContent className="max-h-72 overflow-auto z-[1000]">
                  {layouts.map((l) => <SelectItem key={l.id} value={l.id} className="text-[12px]">{l.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="pt-2">
              ...
              <input
                type="file" accept="image/*" className="w-full text-[12px]"
                onChange={(e) => {
                  const f = e.target.files?.[0] || null;
                  if (!f) { updateConfig("companyLogoDataUrl", null); return; }
                  const r = new FileReader();
              ...
              <Input className="h-8 text-[12px]" value={node.config.companyLogoUrl || ""} onChange={(e) => updateConfig("companyLogoUrl", e.target.value)} placeholder="https://example.com/logo.png" />
            </div>
          </div>
        );
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <Badge variant="secondary" className="text-[10px] py-0 px-1">{node.type} - {node.subtype}</Badge>
      </div>
      <div>
